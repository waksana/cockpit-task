...
const descriptions = {
  task_read: 'Read a bounded Task view. Use overview for coordination and execution for complete current requirements. History is requested separately. Reads never acknowledge. Include your actor_session_id to check your current assigned Task too.',
  ...
  task_assign: 'Assign an unassigned Task to an existing capable Executor and send one Task reference. Checks capability without installing it. Does not interrupt or queue instructions. Inspect durable step results before any recovery.',
  task_edit: 'Edit the complete current description or Task metadata. Description revisions keep a changelog. Only an edit reported as the assigned Executor automatically acknowledges its own new revision. Does not send messages or change execution status.',
  ...
  task_report: 'Append activity against an acknowledged revision, explicitly change execution status, or deliver an outcome. Stale activity may save while stale status/outcome are rejected. Inspect per-field results; never relabel old work as a new revision.',
