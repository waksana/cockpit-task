...
const descriptions = {
  task_read: 'Read bounded Task data. overview+include combines only needed latest groups in one consistent read. For list, explicitly filter by owner, executor or parent_task_id; actor_session_id checks your assigned Task but is not an automatic list filter or authentication. Executor starts/resumes with execution for full requirements; selection cannot replace it. Context has blocked_by, ready, parent/depth. Histories, dependency_notices and automation_log stay paginated. Reads never acknowledge.',
  task_create: 'Register a Task with complete requirements. Optional automation selects a registered script and typed parameters, saving an immutable snapshot, no Agent Executor; Linux/WSL2 only, else AUTOMATION_PLATFORM. Never executes or assigns: optionally subscribe, then task_automation_start. Optional blocked_by: same-Owner Tasks all done before assign/start. An owner executing an Agent Task creates its child (parent_task_id; max 3 levels, DELEGATION_DEPTH_EXCEEDED). Keep request_id stable on retry.',
  ...
  task_session_create: 'Create a real session with Owner and Executor Task roles; readiness checks Executor capability. Optionally prepare explicitly selected existing skills and mcp_servers; no automatic matching, installation or authentication. Omitted selections preserve ordinary creation. Resource-aware creation requires host preparation v1 before creating. Does not bind a Task or send a prompt. Inspect per-step effects and readiness; never recreate on an unknown result.',
  ...
  task_assign: 'Assign an unassigned Agent Task to an existing capable Executor and send one assigned reference. Rejects automation. Checks capability and idle/empty state without installing capability or proactively interrupting. TASK_NOT_READY until blocked_by is done. Titles an auto-named Executor (session_title). The checks and enqueue send are not atomic: a race may return queued or unconfirmed. Inspect durable per-step results and failure-time availability_reasons; never blindly resend.',
...
export function createMcpRoutes({ execute, schemas, signal, report }) {
  ...
  async function connection() {
    const server = new Server({ name: 'cockpit-task', version: '0.1.12' }, { capabilities: { tools: {} } });
