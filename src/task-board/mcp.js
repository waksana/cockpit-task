...
const descriptions = {
  ...
  task_automation_start: 'Explicitly enqueue a created automation Task once, after optional status subscription. The module executes one script at a time without Agent polling. Rejects TASK_NOT_READY while a blocked_by Task is not done. Freezes the definition until execution ends. Success writes done+outcome, failure/interruption blocked+outcome; no retry. Replaying a request never reruns the script.',
  ...
  task_assign: 'Assign an unassigned Agent Task to another existing capable Node session (its assignee) and send one "Task assigned" reference. Rejects automation, SELF_ASSIGNMENT (the orchestrator itself) and DELEGATION_CYCLE (ancestors). Checks capability and idle/empty state without installing capability or proactively interrupting. TASK_NOT_READY until blocked_by is done. Checks and send are not atomic; a race may return queued or unconfirmed. Inspect durable per-step results and failure-time availability_reasons; never blindly resend.',
  task_edit: 'Replace the complete description or edit Task metadata. Description changes keep a changelog. Only an actual description change by the assignee on its unfinished Task automatically acknowledges that revision; unchanged text and metadata-only edits do not. Queued/running automation rejects edits; script and parameter snapshots are immutable. blocked_by replaces the complete blocker set only while the Task awaits dispatch. Silent by default; notify_assignee:true makes the service send the assignee one fixed immediate [Task updated] card for a changed description of an assigned unfinished Agent Task (else rejected before saving). Put every explanation in the definition; no free-text notes. Never changes execution status; inspect notifications/notification_error separately from saved effects.',
  task_ack: 'Record that the assignee has read the current description revision. Automation is service-managed and rejects ACK. Does not start work, add activity or send messages.',
  task_reopen: 'Explicit user rework: the original ready assignee reopens its done Agent Task to in_progress with full description/reason and a new auto-ACKed revision, even for identical text. Requires post-upgrade assignment tracking, no later assignment or unfinished Task, fresh revision/context and safe retained workspace. Preserves history; old outcomes/retros become historical. No cancelled/automation reopen, dispatch or subscription renewal. Keep request_id stable.',
  task_report: 'Assignee reports against an acknowledged revision. Automation rejects Agent reports. done requires a new outcome in the same request and explicit retro: useful text or null for no findings; omission is rejected. Stale activity may save while stale status/outcome/retro are rejected. Only subscribed transitions, waiting-dependent notices and Subtask done/blocked notices to the parent Task\'s assignee notify the orchestrator. Inspect notification_error separately from saved effects; never blindly resend.',
  task_cancel: 'Cancel an unfinished Task with a reason. Agent sessions are not interrupted. Queued automation never launches; running automation requests process-group termination, but cancelled is not proof of exit or rollback: read run facts/outcome/barrier. Only a subscription targeting cancelled, a waiting dependent of this blocker or a Subtask notice to the parent Task\'s assignee notifies the orchestrator. Cancelled Tasks cannot reopen. Inspect notification_error separately from saved effects.',
  task_subscribe: 'Optional: default to no subscription. Register a one-shot wait only when a target state enables necessary orchestrator follow-up, not progress tracking. Recipient is the Task\'s orchestrator, not the caller. Rejects an already-matching status, terminal Task or duplicate waiting subscription. First matching transition consumes the wait and enqueues a status card. Inspect subscriptions for delivery facts; acceptance does not mean read.',
  task_retro_handle: 'The orchestrator records how a recorded retro with findings (by outcome_id, normally the latest) was handled: fixed, followup (reference required; terminal, never revisited when the follow-up finishes), watching or dismissed, with a note. Only the Task orchestrator (ORCHESTRATOR_REQUIRED); an assignee never handles its own retro. A newer retro after reopen needs its own handling. Rewrites append history. Sends no messages and changes no Task status. List retro=unhandled|watching finds pending ones.',
  task_unsubscribe: 'Cancel a still-waiting Task status subscription using its subscription_id and a stable request_id, including when the planned orchestrator follow-up is no longer needed. Cannot recall a consumed notification. Read subscriptions to inspect the recorded transition and notification result.',
};
...
export function createMcpRoutes({ execute, schemas, signal, report }) {
  ...
  async function connection() {
    const server = new Server({ name: 'cockpit-task', version: '0.1.13' }, { capabilities: { tools: {} } });
