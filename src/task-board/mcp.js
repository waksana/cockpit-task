...
const descriptions = {
  task_read: 'Read bounded Task data. overview+include combines only needed latest groups in one consistent read. For list, explicitly filter by owner, executor or parent_task_id; actor_session_id is not an automatic list filter or authentication. Executor starts/resumes with execution, not a selection. Context: blocked_by, ready, parent/depth. Returns actor_role. retro filter: unhandled|watching. Histories, retro_handlings, notices and automation_log stay paginated. Reads never acknowledge.',
  task_create: 'Register a Task with complete requirements. Optional automation selects a registered script and typed parameters, saving an immutable snapshot, no Agent Executor; Linux/WSL2 only, else AUTOMATION_PLATFORM. Never executes or assigns. Optional blocked_by: same-Owner Tasks done before assign/start. An executing owner creates its child (automatic lineage, no parent_task_id input; max 3 levels, DELEGATION_DEPTH_EXCEEDED); others get DELEGATION_OWNER_MISMATCH. Keep request_id stable on retry.',
  ...
  task_session_create: 'Create a real session with the Node role; readiness checks execution capability. Optionally prepare explicitly selected existing skills and mcp_servers; no automatic matching, installation or authentication. Omitted selections preserve ordinary creation. Resource-aware creation requires host preparation v1 before creating. Does not bind a Task or send a prompt. Inspect per-step effects and readiness; never recreate on an unknown result.',
  ...
  task_reopen: 'Explicit user rework: original ready Executor reopens done Agent Task to in_progress with full description/reason and a new auto-ACKed revision, even for identical text. Requires post-upgrade assignment tracking, no later assignment or unfinished Task, fresh revision/context and safe retained workspace. Preserves history; old outcomes/retros become historical. No cancelled/automation reopen, dispatch or subscription renewal. Actor is attribution, not authentication; keep request_id stable.',
  task_report: 'Report against an acknowledged revision. Automation rejects Agent reports. done requires a new outcome in the same request and explicit retro: useful text or null for no findings; omission is rejected. Stale activity may save while stale status/outcome/retro are rejected. Only subscribed transitions, waiting-dependent notices and child done/blocked notices to the parent Executor notify Owner. Inspect notification_error separately from saved effects; never blindly resend.',
  task_cancel: 'Cancel an unfinished Task with a reason. Agent sessions are not interrupted. Queued automation never launches; running automation requests process-group termination, but cancelled is not proof of exit or rollback: read run facts/outcome/barrier. Only a subscription targeting cancelled, a waiting dependent of this blocker or a child notice to the parent Executor notifies Owner. Cancelled Tasks cannot reopen. Inspect notification_error separately from saved effects.',
  ...
  task_retro_handle: 'Owner records how a recorded retro with findings (by outcome_id, normally the latest) was handled: fixed, followup (reference required; terminal, never revisited when the follow-up finishes), watching or dismissed, with a note. Only the Task Owner; an Executor never handles its own retro. A newer retro after reopen needs its own handling. Rewrites append history. Sends no messages and changes no Task status. List retro=unhandled|watching finds pending ones.',
