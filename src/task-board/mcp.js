...
const descriptions = {
  task_read: 'Read bounded Task data. overview+include combines only needed latest groups in one consistent read; omit include for legacy views. For list, explicitly filter by owner or executor; actor_session_id checks your assigned Task but is not an automatic list filter or authentication. Executor starts/resumes with execution for full requirements; selection cannot replace it. Tasks show blocked_by statuses and ready. Histories, dependency_notices and automation_log stay paginated. Reads never acknowledge.',
  task_create: 'Register an independent Task with complete requirements. Optional automation selects a registered script and typed parameters, saving an immutable snapshot without an Agent Executor; Linux/WSL2 only, else AUTOMATION_PLATFORM. Creation never executes: optionally subscribe, then task_automation_start. Ordinary Tasks remain unassigned. Optional blocked_by lists same-Owner Tasks that must all be done before assignment/start (no self/cycles); it never changes status or dispatches. Keep request_id stable on retry.',
