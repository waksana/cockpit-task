...
const descriptions = {
  ...
  task_create: 'Register a Task with complete requirements. Optional automation selects a registered script and typed parameters, saving an immutable snapshot, no Agent Executor; Linux/WSL2 only, else AUTOMATION_PLATFORM. Never executes or assigns. Optional blocked_by: same-Owner Tasks done before assign/start. An executing owner creates its child (parent_task_id; max 3 levels, DELEGATION_DEPTH_EXCEEDED); nobody else creates for it (DELEGATION_OWNER_MISMATCH). Keep request_id stable on retry.',
