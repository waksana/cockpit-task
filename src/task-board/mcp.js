...
const descriptions = {
  task_read: 'Read bounded Task data. The caller is the session named by host invocation metadata; reads return actor_role (orchestrator, assignee or none) relative to it. overview+include combines only needed latest groups in one consistent read. For list, filter by orchestrator, assignee or parent_task_id, or omit them with status=unfinished for the pre-dispatch conflict check across all Tasks. List conditions are 80-character previews with truncated flags; execution retains complete requirements. An assignee starts/resumes with execution, not a selection. Context includes active blocked_by, derived ready and parent/depth; dependencies is the paginated active/resolved prerequisite history. retro filter: unhandled|watching. Other histories, notices and automation_log stay paginated. operation resolves request_id only in the calling session, shared by its main and subagents. Reads never acknowledge.',
  task_create: 'Register a Task with complete requirements; the calling session becomes its orchestrator. If the caller is executing a Task, the new Task is automatically that Task\'s Subtask (lineage recorded, no parent input; max 3 levels, DELEGATION_DEPTH_EXCEEDED). Optional automation selects a registered script and typed parameters, saving an immutable snapshot, no Agent assignee; Linux/WSL2 only, else AUTOMATION_PLATFORM. Never executes or assigns. Optional blocked_by is the complete active prerequisite set: {task_id} follows one explicit Task execution, while {condition} states a concrete unmet requirement and how it becomes satisfied. Keep request_id stable on retry.',
  ...
  task_automation_start: 'Orchestrator only (ORCHESTRATOR_REQUIRED). Explicitly enqueue a created automation Task once, after optional status subscription. The module executes one script at a time without Agent polling. Rejects TASK_NOT_READY while blocked_by is unmet. Freezes the definition until execution ends. Every finished run writes done plus a truthful outcome; succeeded/failed/interrupted remain separate automation facts and cancellation remains cancelled. No retry. Replaying a request never reruns the script.',
  ...
  task_assign: 'Orchestrator only (ORCHESTRATOR_REQUIRED). Assign an unassigned Agent Task to another existing capable Node session (its assignee) and send one "Task assigned" reference. Rejects automation, SELF_ASSIGNMENT (the orchestrator itself) and DELEGATION_CYCLE (ancestors). Checks capability and idle/empty state without installing capability or proactively interrupting. TASK_NOT_READY until every prerequisite is satisfied or explicitly resolved. Checks and send are not atomic; a race may return queued or unconfirmed. Inspect durable per-step results and failure-time availability_reasons; never blindly resend. A finalized receipt in the same calling session proving assignment=applied and message=not_sent may be finished once with resume_request_id.',
  task_edit: 'Orchestrator or assignee only (ORCHESTRATOR_OR_ASSIGNEE_REQUIRED). Replace the complete description or edit Task metadata. Description changes keep a changelog. Only an actual description change by the assignee on its unfinished Task automatically acknowledges that revision; unchanged text and metadata-only edits do not. Queued/running automation rejects edits; script and parameter snapshots are immutable. blocked_by atomically replaces the complete active prerequisite set on any unfinished Task (automation only before it starts); an assignee may add a concrete textual condition, but only the orchestrator/user may resolve or replace one. Adding the first unmet prerequisite or resolving the last notifies the assigned assignee once unless self-authored; ordinary updates and partial resolution while still blocked are silent. An assignee-created textual blocker notifies its orchestrator once. Put explanations in the definition; there are no free-text notes. Never changes lifecycle status; inspect notifications/notification_error separately from saved effects.',
  ...
  task_report: 'Assignee only (ASSIGNEE_REQUIRED). Reports against an acknowledged revision. Lifecycle writes are in_progress or done; blocking is derived from blocked_by. Automation rejects Agent reports. done requires a new outcome in the same request and explicit retro: useful text or null for no findings; omission is rejected. Stale activity may save while stale status/outcome/retro are rejected. A Task dependency is durably resolved by that done transition only; reopening the blocker cannot reactivate the old relation. Matching subscriptions and necessary dependent/parent notices are deduplicated. Inspect notification_error separately from saved effects; never blindly resend.',
...
export function createMcpRoutes({ execute, schemas, signal, report }) {
  ...
  async function connection() {
    const server = new Server({ name: 'cockpit-task', version: '0.3.1' }, { capabilities: { tools: {} } });
