...
const descriptions = {
  ...
  task_report: 'Report activity against an acknowledged revision, explicit status and/or an outcome. Automation rejects Agent reports. done requires a new outcome in the same request. Stale activity may save while stale status/outcome are rejected; never relabel old work as a new revision. Only an actual transition matching an explicit subscription notifies the Task Owner. Inspect notification_error separately from saved Task effects; never blindly resend.',
