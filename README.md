...
**Cockpit Task 的定位是工具，不是决策者。** 它格式化记录工作事实并提供常用操作；Commander 读取这些记录、判断先后与并发，owner 负责实际结果，用户决定重要取舍。服务保留权限和一致性校验，不替 Commander 制定业务限制或自动推进任务。详见[定位与契约](docs/contract.md#定位供-commander-使用的工作工具)。现有 MCP、skill 和安装技术名称仍保留 `work-commander`。
