...
service-managed: no assignee, ACK, session slot or Agent report. Assignee read/edit/cancel
access does not authorize create/start; only orchestration guidance covers it. Script/inputs never change;
...
There is no identity parameter. The host attaches your session to every call in MCP
`_meta["cockpit/invocation"]`, and the service rejects a call without it
(`INVOCATION_REQUIRED`). Calls from your subagents are attributed to your session and
recorded as subagent calls. Identity is attribution, not an ACL or proof that another
session read the Task. Tool access is not permission to claim another assignee's work or ACK for them.
...
Successful changed-description edits by the assignee on an unfinished
...
The assignee reads full `execution` at start, resume and checkpoints before confirming
...
An ACK reminder describes the assignee's responsibility, not an instruction
for the orchestrator or another reader to ACK on their behalf.
...
It adds no notification, dispatch or orchestrator review gate. The service enforces
...
The retro inherits that outcome's revision, assignee, author, reported source,
...
Task registration, node-session creation/preparation and assignment are distinct.
`task_session_create` assembles a new session with the single Task `node` role (assignee
of its own Task, orchestrator of the Subtasks it may create), optionally preparing explicit
...
An orchestrator can use `task_session_prepare` for an already loaded idle node with its
role applied and no pending role reload. Exclude any assignee bound to unfinished
...
After binding it tries once to title an auto/default-named assignee session with the
...
assignee, and fresh context/revision under a new request ID. Task must still be
...
Do not create a second assignee or resend to manufacture a successful receipt.
An accepted message is not an ACK or evidence of actual work.

## Task dependencies (blocked_by)

`blocked_by` is the complete set of Task IDs (at most 20, any orchestrator) that must all
...
yet started; `[]` clears it. The service rejects an unknown Task, a cancelled blocker,
self-dependency, cycles and an ancestor of the dependent (`BLOCKER_ANCESTOR`: the ancestor
waits for the dependent, so it would deadlock). A blocker may belong to another orchestrator;
notices still go to the dependent's orchestrator, who coordinates the order. Changes refresh `write_context` but not the description
revision, and send nothing.

Ready means every blocker is `done`. `task_assign` and `task_automation_start`
reject `TASK_NOT_READY` otherwise, before any assignee check; there is no override.
Readiness never changes status, assigns, starts or dispatches: the dependent stays
unassigned `todo` and dispatch remains the orchestrator's authorized judgment. Reads show
...
sends one `[Subtask ready](task:<uuid>?event=ready)` card for the dependent to its orchestrator.
When a blocker is cancelled, it sends one `[Subtask blocker cancelled](task:<uuid>?event=blocker_cancelled)`
card; the dependent stays not ready until its orchestrator removes that blocker or cancels the
dependent. Edits that make a Task ready send nothing. A blocker reopened and
...
not be executed before that discussion. Its orchestrator may still cancel it before dispatch on the
user's decision. On its ready card, assign it to a new session rather than claiming it
yourself. That assignee discusses it with the user, records the agreed requirements as its
own Task's complete definition, then delivers directly or delegates more specific
Subtasks. If the user decides against it, cancel it with the user's decision as the reason. This is guidance only: no new status, kind or tool.

## Subtasks of your Task

A session's relation is decided per Task: assignee of the Task assigned to it (ACK, deliver,
report), orchestrator of the Subtasks it creates (state requirements, order, dispatch, follow
up, integrate). Derive it from Task facts, not memory: `assignee` is you means your Task,
`orchestrator` is you means your Subtask; reads return it as `actor_role` (`assignee`,
`orchestrator` or `none`; only rows written before schema v9 may show
`orchestrator_and_assignee`), and cards say `Task …` or `Subtask …`. When unsure, read the
Task. The two stay separate: no self-acceptance, an orchestrator does not implement a
delegated Subtask, and one unfinished assignment per session still holds, so Subtasks go to
other sessions. `task_session_create` gives new sessions the `node` role, so a Subtask's
assignee may split further by the same rules without a reload.
...
the load, the more the session leans toward delegating. A Subtask must be more specific than its
parent, never passed down unchanged, and within the parent's authorized scope; anything
outside that scope is asked of the user directly. Its description is the complete Subtask
agreement and references the parent. Record which Subtask covers what in the parent's
activity. The parent integrates and verifies Subtask results before completing its own Task.
Subtasks are part of the parent's Task: follow each one until its result is integrated.
The service wakes the parent's assignee, as the Subtask's orchestrator, with one `Subtask done`,
`Subtask blocked` or `Subtask cancelled` card (`child_done`, `child_blocked`, `child_cancelled`)
per such transition while the parent is unfinished;
...
the Subtask with one bounded overview and integrate, unblock, revise or replace it within your
...
beneath a depth-3 Task fails with `DELEGATION_DEPTH_EXCEEDED` and saves nothing; deliver
directly or ask the user how to restructure. Lineage is fixed at creation, survives the parent
finishing and is never inferred for Tasks created before schema v7. It records where a
Subtask came from; it does not change `blocked_by` readiness, notices, assignment or authority.
Subtask notices go to the Subtask's orchestrator, which is the parent's assignee. Read direct
Subtasks with `task_read(view=list, parent_task_id=<Task ID>, status=all)`; delivery evidence
for Subtask cards is in `task_read(view=child_notices, task_id=<Subtask ID>)`.

The service rejects relation confusion before any effect:
- `SELF_ASSIGNMENT`: a Task is never assigned to its own orchestrator, for any node; assign
  another session. When new work appears after your Task is done, use authorized reopen or
  another node. Doing the work yourself without a Task fits only a node that already holds a
  Task, or an explicit user instruction, never a root node's default delegation;
- `DELEGATION_CYCLE`: a Subtask is never assigned to a session that orchestrates or is assigned
  one of its ancestor Tasks, so work cannot loop back up the lineage;
- `BLOCKER_ANCESTOR`: a Task is never `blocked_by` its own ancestor.

## One-shot status subscriptions

Default to no subscription. An orchestrator registers only when a future state enables a
concrete, necessary authorized orchestrator follow-up, not simply to track progress, know
completion or repeat a report.
For example, evidence may be needed for an orchestrator decision or another authorized
independent Task; a standalone delivery with no orchestrator action needs no wait.
The orchestrator judges this need without asking the user to name or approve the subscription.
...
completion reminder. The assignee's authorized work never waits for the orchestrator to subscribe
or read a notice.
An assignee's direct user question, even when blocked, stays in that session;
the orchestrator does not subscribe to relay it or turn it into an acceptance step.
...
description; the system notifies the orchestrator when B becomes ready. Keep one-shot
subscriptions for other necessary follow-ups, such as a decision from a result.
Private orchestrator notes, todos and plans trigger no reminder: without a dependency or
subscription, a status-dependent follow-up waits until the user prompts it.

The orchestrator can explicitly register with `task_subscribe`, withdraw with `task_unsubscribe`,
...
Its recipient is derived from the Task's `orchestrator`, not an arbitrary addressee or the
reported actor. This adds no subscription capability to the assignee.
...
The system sends a separate `Subtask status changed` (`status_changed`) card to the orchestrator as a new prompt.
It does not keep the subscribing turn suspended; host enqueue queues it when busy
without interrupting. This is not the `updated` handoff to the assignee: do not clear
...
delivery or manually send a replacement notice to the orchestrator because notification failed.
...
restores default progress/final notifications nor permits assignee-to-orchestrator messages.

## Original-assignee rework after done

Only explicit user authorization for rework permits the original assignee to use
`task_reopen` on its done Agent Task. Read full current `execution`, reconcile the
complete new agreement, and submit `task_id,request_id,write_context,
revision,description,reason`. Description is full text; reason records the user
decision, not an authentication credential. The calling session must equal the recorded assignee;
...
upgrade, no other Task may have been assigned to that assignee since it, and no
...
idleness: the original assignee can call while executing this very user request.
...
`ASSIGNEE_MISMATCH` means the caller differs from the original assignee;
`ASSIGNEE_OCCUPIED` means other unfinished work; `TASK_STATE_CONFLICT` means not done;
...
The orchestrator does not create a replacement when eligible original-assignee continuation
...
effects. Cancelled Tasks cannot resume, and bound Tasks cannot change assignee.
...
the orchestrator; uncertainty must not be disguised as success or safe-to-retry failure.
