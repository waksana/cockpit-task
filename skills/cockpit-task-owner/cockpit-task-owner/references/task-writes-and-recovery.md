...
`task_session_create` assembles a new session with both Task roles (Executor for its
own assignment, Owner for child Tasks it may delegate), optionally preparing explicit
`skills` / `mcp_servers`; it does not assign or start a Task. Readiness checks Executor
capability; sessions created or prepared earlier may carry only the Executor role.
...
up). Owner and Executor guidance stay separate: no self-acceptance, Owner does not implement
a delegated child, and one unfinished assignment per session still holds, so children go to
other sessions. `task_session_create` gives new sessions both roles, so a child's Executor may
delegate further by the same rules without a reload.
...
A child's status or dependency card reaches the parent's Executor as that child's Owner; it
does not change the parent's agreement. Without ready Owner capability, deliver directly or
ask the user; never fake delegation.
...
`task_read(view=list, parent_task_id=<Task ID>, status=all)`.
