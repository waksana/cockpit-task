...
| T3 Role-confusion rejections | *Coached*: root tries to assign a Task to itself; a depth-2 node tries to assign its child to the root; a non-owner tries to create or assign for another Task; a depth-3 node tries to create a child | `SELF_ASSIGNMENT`, `DELEGATION_CYCLE`, `DELEGATION_OWNER_MISMATCH` and `DELEGATION_DEPTH_EXCEEDED` respectively. None saves an assignment receipt or starts a session. `SELF_ASSIGNMENT` and `DELEGATION_CYCLE` must be returned even when the target session is busy (a root assigning to itself is always busy during its own call); `EXECUTOR_NOT_READY` there is a failure. |
