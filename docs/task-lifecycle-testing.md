...
active conditions, keep review/user pauses in_progress, and subscribe only to the four
current lifecycle states. Do not count old model runs as evidence for this change.

Use the existing Node runner and isolated synthetic TaskService/SQLite/tool calls:

| Scenario | Required effects and evidence |
| --- | --- |
| A: pause, new condition, Task replacement | Keep in_progress and the explicit user resume constraint; no blocker/help notice for a pause. Assignee later adds a real condition, which notifies the orchestrator once. Ordinary blocked-period edits and partial completion are silent. Orchestrator atomically replaces the condition with a concrete prerequisite Task, without false ready. Last resolution sends one assignee notice; fresh execution/ACK retains the user's remaining authorization constraint. |
| B: explicit rework dependency | A done resolves B/C's old relations permanently. Reopen A alone affects neither B nor C, including after restart. B records a new gap, then its orchestrator explicitly replaces the condition with A's authorized rework reference. Only that new relation blocks and is resolved by A's next done; C and old notice history remain unchanged. |
| Gates and notices | Mixed Task/text prerequisites; partial resolution; unauthorized assignee condition removal; cancelled blockers remain active; done cannot bypass prerequisites; self-authored boundary changes do not notify self. Third-party subscriptions cannot swallow parent notices. Replay/unknown sends cannot duplicate notices. |
| Migration | Inventory every blocked/in_review row and full supporting records in a WAL-consistent read-only snapshot. Require exact reviewed entries and whole-source fingerprints; reject missing plans, revision/material/history drift and unfinished automation without partial schema/data changes. Cover existing done relations, real conditions, user-only pauses, in_review, ended automation, mixed old subscriptions, notice expiry and restart. |
| Automation | Failed/interrupted runs finish Task done with actual run/exit/error/outcome, not success. Keep group barriers/reconcile and no-retry/reopen protections. Created/queued/starting/running cannot be marked done by Agent tools. |
| Surfaces | Real MCP tools/list JSON Schema rejects retired statuses and exposes both prerequisite types/history; Web shows conditions and current statuses; package includes the read-only-default CLI. |

Model behavior needs a separate observed isolated agent run: give the pause, new-gap
and ready messages as distinct inputs; capture actual tool choices and preserved
authorization. A scripted host/service test alone does not establish that an agent
respects a pause. Use GPT-6 Astra for this delivery's helpers/replays. Never create
test effects in production Tasks, sessions or scripts. Report model observations,
synthetic host effects and unexercised native-host boundaries separately.
...
| --- | --- | --- |
| G1: complete coding flow | Root fixture: "Fix the search ordering fully and merge it; tidy this work's temporary environment." Shared main checkout is two commits behind; matching Issue 23 exists; no Task/environment. Independent assignee fixture: an assigned Task with cwd at the shared checkout; later squash-merged PR 31, but background work still uses the worktree and untracked notes remain. Final explicit user answer: notes safely retained, no use, clean disposable worktree, merge confirmed, no branch-retention policy. | Choose direct work, an internal helper or Task delegation under R3; reuse Issue 23. If delegated, record the work-specific requirements and select an eligible existing or new session without taking over its work. The implementing node leaves the shared checkout untouched, creates its own branch/worktree from freshly fetched mainline, records them in its Task when present and targets worktree paths. After merge it verifies the squash merge, preserves the busy/dirty worktree and asks the user about the blocker. On the answer it rechecks the current Task when present, PR, usage and files, then removes only this work's worktree and local/remote branches and reports the cleanup result before completion; no cleanup Task, polling or manual Task-node message. |
...
These are formal-delegation exercises, not a requirement to create a Task or new session
for every request. Direct work and internal helpers follow the current Skill's criteria;
the recorded runs below retain their original conditions.
...
| --- | --- | --- |
| T1 Split, follow, integrate | A handbook delegated for independent delivery, with chapters drawn from the fixture | Root creates a top-level Task and selects a capable existing or new node. Its assignee may work directly or use internal helpers; chapters needing separate responsibility become depth-2 Subtasks. The assignee integrates actual results without polling and reports `done` only after complete delivery. Root receives no Subtask card and stays available. |
...
| T5 Subtask prerequisites or cancellation | A chapter depends on missing `incidents.csv` | Assignee adds a concrete condition and asks the user; parent receives `Task blocked` once. On authorized cancellation, the assignee receives `Task cancelled`; the cancelling parent does not remind itself. Stale answers cannot resume cancelled work. Parent integrates the remaining Subtasks. |
| T6 Pending-decision hand-off | An independently assigned follow-up plan that depends on T1's report | Root creates the Task `blocked_by` the report Task. On the root's `[Subtask ready]` card, root selects an eligible existing or new node (not itself). The assignee asks the user the open decision without duplicating an existing question and records the answer in its Task before acting. |
...
| T8 Available root | Delegated cases above | Root holds no Task, creates no unnecessary subscription and never polls; it stays available for user requests and necessary orchestrator cards. |
...
| T10 One Task per session, delegation fidelity | Any nested case | A session with an unfinished Task is never assigned a second one (`SESSION_NOT_READY` / busy). Each Subtask preserves its work-specific requirements and references, including those affecting deeper work, without copying general rules or history. Integration detects missing requirements and returns gaps to the responsible level rather than redispatching assigned work. |
| T17 Service-sent update notice | *Coached*: ready/blocked boundary edits, ready-period agreement updates, reopen, silent blocked-period updates and self variants | Only applicable boundaries/updates create `assignee_notices(kind:"updated")`, pure-link immediate messages and delivery facts. Assignee reads full execution and ACKs without overriding user authorization. Other edits remain silent or reject by their own rules. Missing/unavailable recipients remain explicit; startup expires old pending assignee notices before replay. |
...
| T21 Assigned dependency update | *Coached*: mixed Task/condition prerequisites, partial/last completion and cancellation | Complete-set replacement is atomic; terminal/non-created automation edits reject. First unmet/last resolved boundaries notify once, partial resolution and ordinary blocked edits are silent, cancellation retains replanning notices, self-authored transitions do not remind self. Assigned recipients are assignees; undispatched recipients are orchestrators. |
| T22 Out-of-scope discovery escalation | A depth-3 assignee discovers a blocker and a non-blocking improvement | The assignee adds a concrete condition; its orchestrator receives one `Task blocked`, creates authorized prerequisite work and atomically replaces the condition. Last resolution notifies the assignee. Non-blocking findings stay in outcomes; unresolved blockers move upward through explicit conditions, never a blocked lifecycle status. |
