...
Cockpit Task 是被调用的任务数据库与工具服务，不是自主指挥、监督或调度系统。数据库保存工作记录，MCP 提供操作入口，skill 表达 agent 之间便于沟通的协作约定。它不是 MCP 配置数据库，也不因保存了待办就主动执行。记录主要供讨论 Commander 阅读和理解，也供 owner 与用户按权限查看；目标含义、优先顺序、并发安排、业务冲突和结果是否满足需求，最终由 Commander／owner 根据实际情况判断，重要取舍与授权由用户决定。
...
服务接口契约与 agent 协作约定不是同一层。身份、版本、参数形状及必要转换由服务校验；何时值得汇报、正文中解释什么、如何理解结果属于 agent 的协作判断，由 skill 指导，不要求服务理解自然语言。当前接口仍有 accepted/result/deliver 等固定操作及结果入口要求；它们不是“任意内容都可写”的通用数据库接口，也不应继续扩展成详细业务表单。调整这些现有约束需要明确的接口变更，本节不静默取消它们。
