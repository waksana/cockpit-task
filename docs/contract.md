...
Cockpit Task 是趁手的记录与操作工具，不是自主指挥或调度系统。它按固定契约保存任务、授权、依赖、进展和结果，提供登记、查询、派单及交付等常用操作。记录主要供讨论 Commander 阅读和理解，也供 owner 与用户按权限查看；目标含义、优先顺序、并发安排、业务冲突和结果是否满足需求，最终由 Commander／owner 根据实际情况判断，重要取舍与授权由用户决定。
