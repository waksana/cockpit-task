...
| 普通更新与强制对齐 | 普通更新只改 Task；非常重要的更新可明确先入队一次 Task ID，再调用本体队列推进 MCP，不自动触发 |
| 其他来源的队列 | 保留消息，由原生接续；工具动态追最新队尾，后续队列耗尽后保留最后轮次执行，不要求 idle，不删除或复制重发 |
...
用于判断的原生动态和 Task 自报动态必须区分，未知仍是未知。普通修订只更新 Task；非常重要的更新由 Owner 明确入队一次 Task ID，再调用本体队列推进功能，逐轮中断并保留其他来源消息，动态追最新队尾。后续队列耗尽后停止中断，保留最后接续轮次执行，不要求整个 session idle。修订本身仍不发消息，也不触发循环。
