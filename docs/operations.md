...
依赖功能 v1.2 使用 schema v3：v1/v2 首次启动时以事务追加 dependencies 表、外键和索引，不改既有表内容，不回填假依赖或执行回执。v3 关系只存任务引用、可空的绑定正式版本、可选说明和分页序号；就绪不存盘。旧 v1.1 程序会拒绝 v3，不能直接切旧二进制降级，也不能为降级覆盖线上数据库。迁移前保留一致备份及配套凭证；未知/在途操作遵循原恢复规则，不能因升级自动重放。

执行安装前须确认本服务没有 running 的投递/通知 operation；有在途工作先等待其真实完成或明确处理，不强停。schema 迁移本身不会重置操作；异常中断的 running 仍按原启动恢复语义成为 failed/unknown。升级只涉及 Work Commander，不重启 Cockpit 或中断 owner 会话。
