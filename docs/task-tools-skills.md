...
**普通更新不积累 queue：** 普通要求只更新 Task，不排队发送 cue、提醒或追加要求。非常重要的更新允许明确先把 Task ID 放入原生队列，再调用本体的队列推进 MCP。首次派单仍不擅自中断忙碌目标，不把这项例外变成普通指派默认行为。

**节点 3 的最新决定：** 前端与 MCP 新建 session 共用角色选择和装配；系统按角色注入工具。有相应 MCP 就可调用，包括操作其他 Task，不按 owner / executor 归属鉴权。队列推进作为本体通用 MCP 设计，内部组合原生能力，不宣称原子控制或排除其他来源的新消息。

**创建时角色组合：** 同一模块也可多选，Owner / Executor 可同时具备；
工具取并集、相同资源去重，两份角色 Skill 按委派和执行行为配合。
这不是运行中追加角色，也不放宽单 session 同时最多承接一项未结束 Task。

**其他来源队列的处理：** 不要求 Executor 永远没有 queue。保留已有消息，逐轮中断并由原生接续；用户选择始终追到运行期间最新的队尾，不固定最初目标。后续队列耗尽时保留最后接续轮次继续执行，不要求 idle，也不保证最后一条仍是 Task ID。不删除或复制重发消息。宿主已实现 `cockpit_advance_queue({action:"start"|"get"|"cancel",session_id,operation_id?})`；它不是 Task 业务工具，也不因普通修订自动启动。
...
Task 修订工具不自带隐式中断。重要更新的建议流程为：Owner 明确入队一次 Task 引用，再调用本体通用推进 MCP。工具动态追最新队尾，后续队列耗尽时停止中断，保留最后轮次；不复制 description、不重发已有消息、不冒充最终工作已完成。具体宿主契约见节点 3 文档。
...
| Owner 因重要更新明确推进队列 | Task ID 入队一次，再推进到动态最新队尾；保留最后轮次，不等待 idle |
| 推进期间又有新消息 | 本次调用继续追最新队尾；返回之后不继续监听 |
...
5. 普通修订只提交完整新 description 和原因。Owner 判断更新非常重要、不能等待时，才明确将 Task ID 入队一次并调用本体队列推进 MCP；保留其他来源消息，动态追最新队尾，最后轮次继续执行，不要求 idle。
...
通用角色注册、前端 / MCP 创建选择、能力装配及本体队列推进 MCP 已实现，不新增逐 Task 鉴权。分代 write_context、逐版确认、持久化回执、`resume_request_id` 的未发送操作恢复和引用卡片已在后续技术收口中实现，详见 [实现契约](task-implementation.md)。恢复操作不是恢复终态 Task，未知发送不能据此重试。
