...
Its module ID and HTTP MCP key are `cockpit-task`, source preparation version `0.1.7`.
...
`dist/cockpit-task-0.1.7.tgz` contains runtime dependencies, backend/frontend assets,
...
```sh
pnpm module install /absolute/path/to/cockpit-task-0.1.7.tgz --trust-local-code
```
