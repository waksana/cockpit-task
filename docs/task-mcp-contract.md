...
实现采用以下八个工具。用户最新决定去掉续办与改派，保留独立的执行 session 创建入口：Task MCP 负责创建并配置执行能力，指派仍独立。Task 模块仅提供 Owner / Executor 两份角色技能，不包含 Coding / Research 工作技能。
...
| `task_cancel` | 是 | 是 | 明确取消，不停止 session 或发消息 |
...
所有变更工具都带 `request_id`；对已有 Task 的变更还带 `task_id` 和对应的 `write_context`。`task_create` 和独立的 `task_session_create` 不要求已有 Task 上下文。重复请求不重复产生副作用；同 ID 换输入明确冲突。重放原结果时仍重新进行 definition_check，不因结果缓存而漏掉新修订。
...
取消不是执行成果报告，不应因未 ACK 新 definition 而强迫执行者先继续工作；仍核对生命周期和并发前提。明确置 cancelled，不改变 description、revision、changelog，不冒充执行者 activity，也不自动停止 session 或发送消息。按共同规则返回定义提醒。
...
Executor:
  ...
  task_report(task_id, revision=1,
              status=done, outcome={...}, ...)    → 完整交付，无反向消息
```
