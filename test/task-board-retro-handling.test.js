...
function fixture(t) {
  ...
  const f = {
    ...
    async start(owner, executor) {
      const created = await as(owner, 'task_create', { owner, title: `Task by ${owner}`, description: 'Synthetic requirements' });
    ...
    async finish(executor, id, retro) {
      const done = await as(executor, 'task_report', { ...context(id), status: 'done', outcome: { summary: 'Delivered' }, retro });
    ...
    list: fields => store.read({ view: 'list', owner: 'owner', ...fields }).items.map(item => item.id),
  };
  return f;
}

test('the Owner records fixed, followup, watching and dismissed handling on the latest retro with append-only history', async t => {
  const f = fixture(t);
  ...
  const watching = await f.handle('owner', id, outcomeId, { status: 'watching', note: 'Seen once; decide if it repeats' });
  ...
  const followup = await f.handle('owner', id, outcomeId, {
  ...
  assert.equal(full.handling.author, 'owner');
  assert.equal(f.store.task(id).retro.handling.status, 'followup');

  const same = await f.handle('owner', id, outcomeId, {
    status: 'followup', note: 'Add an install step to github-coding',
    references: [{ label: 'Follow-up', target: 'task:11111111-1111-4111-8111-111111111111' }],
  });
  assert.equal(same.result.status, 'unchanged');
  assert.equal((await f.handle('owner', id, outcomeId, { status: 'fixed', note: 'Fixed in #81', references: [{ label: 'PR', target: 'https://example.invalid/pr/81' }] })).error, null);
  assert.equal((await f.handle('owner', id, outcomeId, { status: 'dismissed', note: 'Not reproducible' })).error, null);
...
test('only the Owner may handle the latest recorded retro that has findings', async t => {
  ...
  assert.throws(() => parseInput('task_retro_handle', { actor_session_id: 'owner', request_id: 'r', task_id: id, outcome_id: outcomeId, status: 'fixed', note: 'x', write_context: 'x' }));

  const empty = await f.start('owner', 'quiet');
...
test('reopened delivery needs its own handling while the old handling stays in history', async t => {
  ...
  assert.equal((await f.handle('owner', id, first, { status: 'fixed', note: 'Fixed' })).error, null);
...
test('list filters find unhandled and watching retros across statuses without automation', async t => {
  const f = fixture(t);
  const unhandled = await f.start('owner', 'a');
  await f.finish('a', unhandled, 'Needs handling');
  const watched = await f.start('owner', 'b');
  const watchedOutcome = await f.finish('b', watched, 'Watch me');
  await f.handle('owner', watched, watchedOutcome, { status: 'watching', note: 'Watching' });
  const fixed = await f.start('owner', 'c');
  const fixedOutcome = await f.finish('c', fixed, 'Fixed');
  await f.handle('owner', fixed, fixedOutcome, { status: 'watching', note: 'Watching first' });
  await f.handle('owner', fixed, fixedOutcome, { status: 'fixed', note: 'Then fixed' });
  const quiet = await f.start('owner', 'd');
  await f.finish('d', quiet, null);
  await f.start('owner', 'e');
  ...
  assert.deepEqual(f.store.read({ view: 'list', owner: 'other', retro: 'unhandled' }).items, []);
  const listed = f.store.read({ view: 'list', owner: 'owner', status: 'done' }).items.find(item => item.id === watched);
  ...
  const page = f.store.read({ view: 'list', owner: 'owner', retro: 'unhandled', limit: 1 });
  assert.equal(page.next_cursor, null);
  const all = f.store.read({ view: 'list', owner: 'owner', status: 'all', limit: 1 });
  assert.throws(() => f.store.read({ view: 'list', owner: 'owner', retro: 'unhandled', status: 'all', cursor: all.next_cursor }), error => error.code === 'INVALID_CURSOR');
});

test('schema v8 adds retro handling to v7 data without backfilling and survives restart', async t => {
  const f = fixture(t);
  const id = await f.start('owner', 'worker');
  await f.finish('worker', id, 'Legacy finding');
  f.store.db.exec('DROP TABLE retro_handlings; PRAGMA user_version=7');
  f.restart();
  assert.equal(f.store.db.prepare('PRAGMA user_version').get().user_version, 8);
  assert.deepEqual(f.store.read({ view: 'overview', task_id: id }).retro.handling, { status: 'unhandled' });
  const outcomeId = f.store.task(id).retro.outcome_id;
  assert.equal((await f.handle('owner', id, outcomeId, { status: 'dismissed', note: 'Legacy' })).error, null);
  f.restart();
