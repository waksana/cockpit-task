...
function fixture(t, overrides = {}) {
  ...
  const host = {
    ...
    create: async () => { calls.push('create'); return { sessionId: 'assignee' }; },
    inspect: async () => { calls.push('inspect'); return { ready: true, idle: true, node: true }; },
  ...
  const write = (name, input) => service.execute(name, {
    actor: 'orchestrator', request_id: `preparation-${++sequence}`, ...input,
  });
...
test('create and existing preparation share explicit resources without Task binding or messages', async t => {
  ...
  for (const [name, target] of [
    ...
    ['task_session_prepare', { session_id: 'assignee' }],
  ]) {
    ...
    assert.equal(result.result.operation.session_id, 'assignee');
  ...
  assert.deepEqual(f.calls.filter(call => typeof call === 'object'), [
    { id: 'assignee', ...selections }, { id: 'assignee', ...selections },
  ]);
...
test('preparation rejects busy, unloaded and unapplied roles without native mutation', async t => {
  const f = fixture(t);
  for (const state of [
    { ready: true, idle: false, node: true },
    { ready: false, idle: false, assignee: false },
    { ready: false, idle: true, assignee: false },
  ]) {
    f.host.inspect = async () => state;
    const result = await f.write('task_session_prepare', { session_id: 'assignee' });
    assert.equal(result.error.code, state.idle ? 'NODE_ROLE_REQUIRED' : 'SESSION_NOT_READY');
...
test('unfinished Task excludes preparation even when idle; completed Assignee can be prepared', async t => {
  ...
  const rejected = await f.write('task_session_prepare', { session_id: 'assignee' });
  assert.equal(rejected.error.code, 'ASSIGNEE_OCCUPIED');
...
test('native partial effects and creation ID survive readiness failures and replay', async t => {
  const native = prepared('assignee', {
  ...
  const input = { actor: 'orchestrator', request_id: 'partial-create', cwd: '/synthetic', skills: ['synthetic-work'] };
  ...
  assert.equal(first.result.operation.session_id, 'assignee');
...
test('lost preparation result remains durable unknown with no retry after restart', async t => {
  ...
  const input = { actor: 'orchestrator', request_id: 'unknown-prepare', session_id: 'assignee' };
...
test('initial null metadata can be prepared; native success still needs final Assignee readiness', async t => {
  const f = fixture(t);
  for (const after of [{ ready: false, idle: true }, { ready: true, idle: false }, { ready: true, idle: true }]) {
    let inspections = 0;
    f.host.inspect = async () => ++inspections === 1
      ? { ready: false, idle: true, node: true, details: { reasons: ['Native metadata uninitialized'] } }
      : { ...after, node: true };
    const outcome = await f.write('task_session_prepare', { session_id: 'assignee' });
    ...
    assert.equal(outcome.error?.code ?? null, !after.ready ? 'CAPABILITY_UNAVAILABLE' : !after.idle ? 'SESSION_NOT_READY' : null);
  }
});
...
test('cancellation before resource mutation preserves a newly created ID', async t => {
  ...
  const f = fixture(t, {
    inspect: async () => { controller.abort(); return { ready: false, node: true, idle: true }; },
  });
  const result = await f.service.execute('task_session_create', {
    actor: 'orchestrator', request_id: 'cancel-created', cwd: '/synthetic', skills: [],
    ...
    await t.test(phase, async t => {
      ...
      const f = fixture(t, {
        create: async () => {
          ...
          return { sessionId: 'assignee' };
        },
        inspect: async () => { inspections++; return { ready: true, idle: true, node: true }; },
        prepare: async () => { controller.abort(); return prepared(); },
      });
      const result = await f.service.execute('task_session_create', {
        actor: 'orchestrator', request_id: `cancel-${phase}`, cwd: '/synthetic', skills: [],
      }, { signal: controller.signal });
      assert.equal(result.error.code, 'REQUEST_CANCELLED');
      assert.equal(result.result.operation.creation, 'created');
      assert.equal(result.result.operation.status, 'partially_applied');
...
test('preparation and assignment reject concurrent same-target work without duplicate effects', async t => {
  ...
  const pending = f.write('task_session_prepare', { session_id: 'assignee' });
  await entered;
  const competing = await f.write('task_session_prepare', { session_id: 'assignee' });
  assert.equal(competing.error.code, 'SESSION_OPERATION_IN_PROGRESS');
  const assignment = await f.write('task_assign', {
    task_id: created.result.task_id, revision: 1, assignee: 'assignee', write_context: created.result.write_context,
  });
  assert.equal(assignment.error.code, 'SESSION_OPERATION_IN_PROGRESS');
  assert.equal(assignment.result.operation.assignment, 'not_applied');
  assert.equal(f.store.task(created.result.task_id).assignee, null);
  release(prepared());
  assert.equal((await pending).error, null);
  f.host.prepare = async () => prepared();
  assert.equal((await f.write('task_session_prepare', { session_id: 'assignee' })).result.operation.status, 'applied');
});

test('resource inputs reject unknown fields and duplicate selections without expanding authority', () => {
  const input = { actor: 'orchestrator', request_id: 'schema', session_id: 'assignee' };
  for (const patch of [
...
test('an admitted assignment excludes preparation even before the Task is bound', async t => {
  ...
  f.host.inspect = async () => {
    ...
    return { ready: true, idle: true, node: true };
  };
  ...
  await started;
  const result = await f.write('task_session_prepare', { session_id: 'assignee' });
  assert.equal(result.error.code, 'SESSION_OPERATION_IN_PROGRESS');
  assert.equal(f.store.task(created.result.task_id).assignee, null);
  assert.deepEqual(f.calls, []);
  release();
...
test('unknown host receipts are not accepted as successful preparation', async t => {
  const f = fixture(t);
  for (const receipt of [null, { ok: true }, prepared('other'), prepared('assignee', { tools: 'pending' })]) {
...
test('pending preparation replay retains the target ID without another observation or mutation', async t => {
  ...
  const input = { actor: 'orchestrator', request_id: 'pending-prepare', session_id: 'assignee' };
  ...
  assert.equal(replay.result.operation.session_id, 'assignee');
...
test('host adapter exposes preparation capability and maps only selected public resources', async () => {
  ...
  const host = {
    ...
    call: async (name, body) => {
      ...
      if (name === 'roles/readiness') return {
        sessionId: 'assignee', ready: false, loaded: true, rolesNeedReload: false,
        appliedRoles: [{ moduleId: 'cockpit-task', roleId: 'node' }], reasons: ['Native metadata uninitialized'],
      };
      if (name === 'session/get') return { meta: {
        sessionId: 'assignee', loaded: true, status: 'idle', nativeProcessing: false, activeOperations: 0, queue: [],
      } };
      assert.fail(name);
    },
  };
  ...
  const observed = await adapter.inspect('assignee');
  assert.equal(observed.node, true);
  assert.equal(observed.ready, false);
  await adapter.prepare('assignee', { skills: ['synthetic-work'], mcp_servers: [{ name: 'tools', tools: ['read'] }] });
  assert.deepEqual(calls.at(-1), {
    name: 'session/resources-prepare',
    body: { sessionId: 'assignee', skills: ['synthetic-work'], mcpServers: [{ name: 'tools', tools: ['read'] }] },
  });
