...
test('packaged Task integrates with real isolated host roles, native SDK and HTTP MCP', {
  ...
  const archive = join(repository, 'dist', 'cockpit-task-0.1.11.tgz');
  ...
  try {
    ...
    assert.equal(installed.manifest.version, '0.1.11');
    ...
    assert.deepEqual(bootstrap.active, [{ id: 'cockpit-task', version: '0.1.11', digest: installed.digest }]);
