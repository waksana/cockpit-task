...
test('packaged Task integrates with real isolated host roles, native SDK and HTTP MCP', {
  ...
  const packageVersion = process.env.ROLLING_SEQUENCE ? `0.0.0-rolling.${process.env.ROLLING_SEQUENCE}` : '0.0.0-dev';
  const archive = join(repository, 'dist', `cockpit-task-${packageVersion}.tgz`);
  ...
  try {
    ...
    assert.equal(installed.manifest.version, packageVersion);
    ...
    assert.deepEqual(bootstrap.active, [{ id: 'cockpit-task', version: packageVersion, digest: installed.digest }]);
    ...
    for (const asset of ['./read-resource.js', './icons.js', './licenses/lucide.txt']) {
      const url = new URL(asset, `${origin}${bootstrap.modules[0].entry}`);
      assert.equal((await app.inject(url.pathname)).statusCode, 200, `${asset} must be a declared packaged asset`);
    }
